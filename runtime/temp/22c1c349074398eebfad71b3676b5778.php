<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:37:"./template/pc/rainbow/user\index.html";i:1495107366;s:38:"./template/pc/rainbow/user\header.html";i:1495107366;s:36:"./template/pc/rainbow/user\menu.html";i:1495107366;s:46:"./template/pc/rainbow/public\footer_index.html";i:1495107366;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
			    		...
			    		<div class="order-list bgno p">
			    			<div class="ddlb-zy">
                              ...
                              foreach($sql_result_item as $key=>$item): endforeach; ?>
                                ...
                                $md5_key = md5("select l.*,c.condition,c.money from __PREFIX__coupon_list l INNER JOIN __PREFIX__coupon c on c.id = l.cid where l.uid = $user[user_id] and  l.use_time = 0 and c.use_end_time > UNIX_TIMESTAMP(NOW())  order by l.id desc limit 2");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select l.*,c.condition,c.money from __PREFIX__coupon_list l INNER JOIN __PREFIX__coupon c on c.id = l.cid where l.uid = $user[user_id] and  l.use_time = 0 and c.use_end_time > UNIX_TIMESTAMP(NOW())  order by l.id desc limit 2"); 
