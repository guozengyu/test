...
class Distribut extends Base {
     ...
     */
    public function set(){
        return $this->fetch('system/distribut');
...
//        exit;
    ...
    public function change(){
        ...
        switch ($id){
            case 0:
                $data=db('config_new')->select();
                break;
            case 1:
                $data=db('config_county')->select();
                break;
            case 2:
                $data=db('config_city')->select();
                break;
            case 3:
                $data=db('config_save')->select();
                break;
        }
        return json($data);
    }
     ...
     * 分销设置
     */
    public function updayte_distribut(){
        $data=input('post.');
        switch ($data['type']){
            case '0':
                $db=db('config_new');
        ...
        foreach ($data as $key=>$val){
            $datas=$db->where('name',$key)->update(['value'=>$val]);
        }
        $this->success('成功！');
    }
