...
class DistributLogic //extends Model
{
     public function hello($type){
         ...
         $distribut=$db->select();
         foreach ($distribut as $val){
             if($val['name']=='pattern') $pattern=$val['value'];
             if($val['name']=='first_rate') $first_rate=$val['value'];
             if($val['name']=='second_rate') $second_rate=$val['value'];
             if($val['name']=='third_rate') $third_rate=$val['value'];
         }
     }
      ...
      * 生成分销记录
      */
     public function rebate_log($order,$type)
     {       
